...
2. 訪客選擇「警示戶＝是」時，前端立即顯示無法辦理提示並返回首頁；同一瀏覽器會保存限制並停用再次進入申請頁，即使再次從廣告開啟落地頁亦然。
3. 只有「警示戶＝否」的訪客可提交姓名、年齡、電話與已同意資料使用。
4. 前端呼叫 `POST /api/leads`，服務端將資料存入 `leads`，初始狀態為「新線索」；服務端同樣拒絕任何「警示戶＝是」請求，避免繞過前端建立線索。
5. 成功後顯示諮詢編號；訪客點擊加入 LINE 時，前端呼叫 `POST /api/leads/:id/line-click` 將狀態更新為「已點擊加入 LINE」：`https://lin.ee/rDrSnEV`。
6. CRM 專員將線索標記為已聯絡、有效諮詢、成交或無效。

CRM 預設顯示臺灣時區（UTC+8）的當日線索；可按提交日期與警示戶狀態篩選，統計卡會顯示該日成功提交總數及警示戶「是／否」數量。
